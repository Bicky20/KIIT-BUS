...
const BusTracking: React.FC<BusTrackingProps> = ({
  ...
  // Internal user location state (used when props are not supplied)
  const [internalUserLat, setInternalUserLat] = useState<number | undefined>(
    undefined,
  );
  const [internalUserLng, setInternalUserLng] = useState<number | undefined>(
    undefined,
  );
  ...
  // Resolved user coordinates: prefer props, fallback to internal watch
  const userLat = propUserLat ?? internalUserLat;
  const userLng = propUserLng ?? internalUserLng;

  // Start internal watchPosition whenever props aren't provided
  useEffect(() => {
    // Always watch internally so user's location is continuously updated
    ...
    watchIdRef.current = navigator.geolocation.watchPosition(
      (pos) => {
        setInternalUserLat(pos.coords.latitude);
        setInternalUserLng(pos.coords.longitude);
        setGeoError(null);
      },
      (err) => {
        // Only show error if props are also not available
  ...
  // Helper to create/update the user location marker on the map
  const updateUserMarker = useCallback((lat: number, lng: number) => {
    ...
    if (userMarkerRef.current) {
      ...
      const userMarker = L.marker([lat, lng], { icon: userIcon }).addTo(
        mapRef.current,
      );
  ...
  // Update user marker whenever resolved userLat/userLng changes
  useEffect(() => {
    if (userLat !== undefined && userLng !== undefined) {
      updateUserMarker(userLat, userLng);
      if (geoError) setGeoError(null);
    }
  }, [userLat, userLng, updateUserMarker, geoError]);

  // FIX: Field 1 = latitude, Field 2 = longitude
  const fetchGPS = useCallback(async () => {
    try {
      const res = await fetch(THINGSPEAK_URL);
      if (!res.ok) throw new Error("ThingSpeak fetch failed");
      const data = await res.json();
      const lat = Number.parseFloat(data.field1); // field1 = latitude
      const lng = Number.parseFloat(data.field2); // field2 = longitude
      ...
      <main className="flex-1 flex flex-col max-w-lg mx-auto w-full px-4 py-5 gap-4 relative z-10">
        ...
        {userLat !== undefined && userLng !== undefined && (
          <div
            className="rounded-xl px-4 py-3 flex items-center gap-3"
            style={{
              background: "oklch(0.96 0.04 240 / 0.95)",
              border: "1px solid oklch(0.72 0.12 240 / 0.4)",
            }}
          >
            <div
